  ...
  <div class="page-grid">
    <section class="card">
      <div class="section-head">
        ...
        <el-button v-if="listMode !== 'followed'" type="primary" @click="createDialogVisible = true">
          ➕ 新建项目
        </el-button>
      </div>
    ...
    <section class="card project-table-card">
      <div class="project-table-wrap">
        <el-table
          ...
          height="100%"
        >
          <el-table-column label="序号" width="80" type="index" :index="projectIndexMethod" />
        ...
        <div class="form-two-cols">
          ...
          <el-form-item label="项目类型">
            <el-select v-model="projectForm.project_type">
              <el-option label="公司项目" value="work" />
...
const listMode = computed(() => {
  if (route.path.includes("followed-projects")) return "followed";
  if (route.path.includes("company-projects")) return "company";
  return "my";
});
const pageTitle = computed(() => {
  if (listMode.value === "followed") return "⭐ 关注项目";
  if (listMode.value === "company") return "🏢 公司项目";
  return "📁 项目清单";
});
...
function projectIndexMethod(index) {
...
async function loadTagList() {
  try {
    const { data } = await fetchProjectTags({
      ...
      scope: listMode.value === "company" ? "company" : "my",
    });
